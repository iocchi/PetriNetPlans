...
namespace pnpros
{
	static actionlib::ActionClient<PNPAction>* pnpac = NULL;
    ...
    if (k != string::npos) {
      ...
      name = nms;
		...
		// cerr << "Exec: " << robotname << "#" << name << " " << params << endl;
	}
	
	void ActionProxy::actionTerminationCallback(const ActionFinished::ConstPtr& message)
	{
		ROS_INFO_STREAM("Deleting " << message->id);
		activeActions.erase(message->id);
	}
	
	void ActionProxy::transitionCb(actionlib::ClientGoalHandle<PNPAction> gh)
	{
	...
	void ActionProxy::feedbackCb(actionlib::ClientGoalHandle<PNPAction> gh, const PNPFeedbackConstPtr& feedback)
	{
...
#if USE_ACTIONLIB
		if (pnpac == NULL) pnpac = new actionlib::ActionClient<PNPAction>("PNP");
