...
void PNPActionServer::ActionExecutionThread(PNPAS::GoalHandle gh) {
    PNPros::PNPGoal goal = *gh.getGoal();
...
bool PNPActionServer::EvalConditionWrapper(PNPros::PNPCondition::Request  &req,
         PNPros::PNPCondition::Response &res)  {
    int r1 = evalCondition(req.cond);
    int r2 = check_for_event(req.cond);

    int result=-1;
    if (r1==-1) result=r2; else result=r1;
...
bool PNPActionServer::GetEventStartingWith(PNPros::PNPLastEvent::Request  &req,
...
bool PNPActionServer::ClearBuffer(PNPros::PNPClearBuffer::Request  &req,
...
bool PNPActionServer::GetVariableValue(PNPros::PNPGetVariableValue::Request  &req,
...
bool PNPActionServer::SetVariableValue(PNPros::PNPSetVariableValue::Request  &req,
...
//           cerr << endl;
//           cout << "Found event " << cond << " in eventBuffer" << endl;
//           for(vector<Event>::const_iterator i = eventBuffer.begin(); i != eventBuffer.end(); ++i)
//           cout << i->eventName << " " << i->time << endl;
//           cout << "buffer current length:" << eventBuffer.size() << endl;
//           cerr << endl;
